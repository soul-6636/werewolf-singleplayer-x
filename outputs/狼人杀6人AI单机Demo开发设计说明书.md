...
| 失败 | 处理 |
|---|---|
| 网络/Provider 超时 | 1 次，指数退避；仍失败则暂停对局并显示错误 |
| JSON 解析失败 | 1 次，附带格式错误；仍失败则暂停对局 |
| 目标已死亡 | 不重复请求旧结果，暂停对局并要求重新处理 |
| 女巫动作与库存冲突 | 1 次提示剩余动作；仍失败则暂停对局 |
| AI 发言为空或过长 | 1 次压缩；仍失败则暂停对局 |

任何失败处理都不能切换到本地 Bot 或使用玩家不该知道的真值信息；对局必须暂停并向玩家显示可操作错误。
...
- 角色分配、玩家座位、AI 人格使用带 seed 的 RNG；人类座位与角色分别随机。
...
- 返回未知 action、死人目标、空文本、超长文本时，能重试，仍失败则暂停对局。
...
| AI 结构化动作 | 合法动作成功率不低于 99%，其余进入错误暂停 |
...
退出条件：配置线上 Provider 后能用脚本完成 100 局固定种子模拟。

### M2：线上 AI 与前端

交付：线上 AI 调用、座位号桌面、狼队私聊、私密/公开视图、决战与遗言面板、自爆动作、结算页和开发揭示。

退出条件：配置线上 Provider 后能在浏览器里完整玩一局；未配置时不能开局。

### M3：模型 Provider

交付：云端 Provider、每 AI 独立 `AgentMemory`、`ClaimGraph`、信息边界和动机分析、认知更新、有限二阶信念、信息披露策略、受控试探、狼人欺骗账本、发言意图、压力响应、换位假设、局势模拟、策略规划/发言生成分层、轮次摘要、结构化输出校验、重试、错误暂停、SSE 发言流和调用日志。
...
| AI API 不稳定 | Provider 超时、一次重试、可见错误并暂停对局；不切换本地 Bot |
...
- [ ] 夜后和票后均能正确结束游戏。
- [ ] 模型输出非法时不会破坏游戏状态。
- [ ] 网络/Provider 失败时有可见错误并暂停对局，不切换本地 Bot。
- [ ] 可以保存、读取并回放一局。
